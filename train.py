...
def main(args):    
    ...
    os.makedirs(args.save_path, exist_ok=True)
     ...
     # Create a log file to store the losses
    loss_log_path = os.path.join(args.save_path, "training_loss_log.txt")
    with open(loss_log_path, "w") as log_file:
        log_file.write("Epoch,Iteration,Loss\n")  # Write the header
    ...
    for epoch in range(args.epoch):
        for i, batch in enumerate(dataloader):
            ...
            with open(loss_log_path, "a") as log_file:
                log_file.write(f"{epoch + 1},{i + 1},{loss.item():.6f}\n")
            if i % 50 == 0:
